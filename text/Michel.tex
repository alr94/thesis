...
% TODO: talk about flattening of training distribution
...
test, and validation sets in the ratio \mccorrect{80:10:10}. In total 
around 15,000 images were generated for the training, test, and validation 
datasets. 
...
simulated Michel electron sample. The reconstructed ionisation was compared to
...
simulated Michel electron events. A linear scaling factor was used to correct 
the reconstructed energy, this was based on a linear fit to the reconstructed 
vs true ionisation energy distribution.
\begin{figure}
	\centering
...
After scaling the reconstructed energy with a linear scaling factor, the energy
resolution and bias of the energy reconstruction were estimated. The overall
energy resolution and bias were estimated by fitting a gaussian to the 
fractional difference between the reconstructed ionisation energy and the true 
ionisation energy, which we define as
\begin{equation*}
	\mbox{Fractional Difference} = \frac{E_{reco} - E_{true}}{E_{true}}.
\end{equation*}
The distribution of fractional differences is shown in Figure
\ref{fig:frac_diff_ion}, along with the gaussian fit, which was restricted to
the central region. Based on this fit, the overall energy resolution was
estimated to be on the order of 10\%, and the bias was estimated to be on the
order of 1\%. 
\begin{figure}
	\centering
...
The distribution fractional energy difference shown in Figure
\ref{fig:frac_diff_ion}, has a significant tail corresponding to under
reconstructing the Michel electron energy. In order to investigate this tail
further, the energy resolution and bias as a function of the true Michel
electron ionisation energy were calculated. This was based on the same method
used to predict the overall energy resolution and bias, but the events were
binned in terms of their true ionisation energy. The reconstructed energy
resolution and bias as a function of true ionisation energy are shown in Figure
\ref{fig:res_and_bias_ion}, where the values are based on the fits shown in 
Appendix \ref{ch:energyfits}. 

\begin{figure}
	\centering
	\begin{subfigure}[b]{\textwidth}
		\centering
		\includegraphics[width=0.9\textwidth]{figures/res_v_energy_ion.pdf}
		\caption {Resolution.}
		\label{fig:res_ion}
	\end{subfigure}
	\begin{subfigure}[b]{\textwidth}
		\centering
		\vspace{5mm}
		\includegraphics[width=0.9\textwidth]{figures/bias_v_energy_ion.pdf}
		\caption {Bias.}
		\label{fig:bias_ion}
	\end{subfigure}
	\caption
...
Overall, the Michel electron reconstruction shows good performance in terms of
energy resolution and bias, however, in the high energy tail of the distribution
there is a significant increase in bias. This tail is due to a reduction in 
hit selection efficiency at high energies, which leads to less of the Michel 
electron ionisation energy being reconstructed. This effect begins to impact 
the fractional difference distribution in the 25--35 MeV bin, and becomes 
dominant above 35 MeV. This leads to a significant tail in the fractional 
difference distribution for the 25--35 MeV bin, seen in Appendix 
\ref{ch:energyfits}. No attempt was made to fit this tail, and therefore the 
estimated energy resolution and bias in the 25 -- 35 MeV bin overstates the 
performance in this region. A more conservative estimate, based on the mean and 
standard deviation of the distribution, gives a bias of $-5\%$ and a resolution 
of $16\%$.
...
This can be tested with the simulated data by using the truth information to
...
$0.0288 \pm 0.0012$ and a standard deviation of $0.0756 \pm 0.0011$, 
suggesting that the energy resolution for reconstructed hits contributes 
around 7--8\% to the Michel electron energy resolution. The remaining 
uncertainty comes from the purity and efficiency of the hit tagging algorithm.
...
contains both track--like and a shower--like components, which require a unique
reconstruction algorithm in order to maximise the energy collected from the
events. This chapter has discussed the development of a Michel electron 
reconstruction algorithm, which is based on semantic segmentation of images 
with a convolutional neural network. This is the first time this type of
algorithm has been applied to Michel electron reconstruction in a LArTPC, and it
shows promising results. However, the significant bias introduced ionisation
energies above around 30 MeV limit the applicability of the algorithm. In this
section we will compare the results from this section to those from other LArTPC
experiments, and discuss the implications of the results of this chapter for 
DUNE.

The closest comparison for Michel electron reconstruction comes from the
MicroBooNE LArTPC\citetemp{TODO}; both MicroBooNE and \protodune{} are surface level TPCs, and
the Michel electron energy reconstruction was based on ionisation energy only.
In MicroBooNE the ionisation energy resolution for Michel electrons was found 
to be between 15--25\% in the 0--30 MeV region\cite{Acciarri2017}, which is 
improved to around 10\% with the algorithm developed here. 

LArIAT\citetemp{TODO} is another surface level LArTPC, however they used
scintillation light to measure the Michel electron energy spectra. TODO.

The final comparison with other LArTPCs comes from ICARUS\citetemp{TODO}, 
which was a deep underground detector, and therefore had significantly lower 
backgrounds than in \protodune{}. In ICARUS, TODO.
...
consider increasing the amount of high energy training data, or modifying the 
loss function to place a larger emphasis on efficiency rather than purity. A 
possible modification to the loss function would be to use the $F_\beta$ 
function discussed in Chapter \ref{ch:chargeid}, and tune $\beta$ to favour 
hit tagging efficiency over purity.

In the DUNE far detector, TODO.
