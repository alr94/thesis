...
signature that is neither track--like nor shower--like. As a result, 
...
decay gives rise to a characteristic energy spectrum that has a sharp
...
will discuss an algorithm that was developed to tackle this problem, and its 
...
defined. This sample includes any daughter of the muon candidate that satisfies
...
A U--ResNet is a type of convolutional neural network that is designed to 
...
In semantic segmentation the aim is to return a map of pixels that correspond 
...
Figure \ref{fig:unet_arch}. The network consists of a repeating structure that
contains the following key components:
\begin{itemize}
	\item Convolutional blocks that contain multiple convolutional layers
...
\begin{figure}
	...
	blue line represent the selection cut for hits that were included in the
...
integrated hit charge in ADC, $C_X$ is an X--correction factor that is
dependent on the X (drift) coordinate of the hit within the TPC, $C_{YZ}$ is a
YZ--correction factor that is dependent on the Y (vertical) and Z (beam) 
coordinates of the hit within the TPC, $N$ is a dimensionless normalisation 
factor that normalises the data and MC distributions to give the same 
magnitude, $W_{ion}$ is the ionisation energy of argon in MeV per electron, 
$C$ is a constant conversion factor that has units ADC per electron, and $R$ 
...
$dQ/dx$ dependent factor that depends on the conditions in the liquid
...
that may be developed for supernova neutrino energy reconstruction in the 
future. 
...
noise model and space charge model in the simulation, which will impact the
U--ResNet scores due to the effects discussed in Section \ref{hittag_syst}.
These effects could be mitigated with a network that has been trained on images
...
have a signature that contains both track--like and a shower--like 
components. Therefore, they require a unique reconstruction algorithm in 
...
performance of this algorithm showed an improvement in energy resolution over 
similar experiments, with an ionisation energy resolution of around 10-15\%.  
However, there is still room for improvement, particularly in the high energy 
...
that, with further study, the semantic segmentation approach could prove to be 
an effective algorithm for reconstructing low energy particles in LArTPCs. In 
...
training sample with enough data. Therefore, this remains as a possible 
...
While Michel electrons provide an interesting sample for studying low
energy electron reconstruction, it is important to note the differences between
Michel electron and supernova neutrino events in LArTPCs, as well as the
...
events. This aids event selection but also masks some of the Michel 
electron energy due to the presence of the Bragg peak of the muon. 

Secondly, the peak of the Michel electron energy spectrum is around 40 MeV as 
...
than supernova neutrinos. Therefore, the energy reconstruction results in the 
...
deposits to be utilised during energy reconstruction in DUNE, which was not 
...
reconstruction in LArTPCs, one that outperforms the current supernova neutrino 
energy resolution estimates based on ionisation energy deposits in the region of
interest for supernova neutrinos. This would be an interesting approach to 
consider when studying the capabilities of DUNE for supernova neutrino 
reconstruction, and may lead to improved energy resolution in the associated
analyses when compared to the benchmark set in the technical design report.
