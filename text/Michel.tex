...
hits in Michel electron candidates for \protodune{} data and simulation,
normalised by the number of primary muon candidates. There is a greater spread
in the fraction of Michel--like hits in data for low scores, but a good
agreement at higher scores above around 0.6. Events are selected if the best
Michel electron candidate contains more than 80\% of Michel--like hits, this
threshold was selected by setting a requirement of greater than 97.5\% purity
on the event selection algorithm in simulation.
\begin{figure}
	\centering
...
those from the epoch before the epoch when the validation score first increased.
...
selection thresholds in the range $[10^{-7}, 1 - 10^{-7}]$, which returned
purities in the range 0.91--0.93, and completeness in the range 0.94--0.96. For
thresholds outside of this range the purity and completeness remained constant,
except for at thresholds of 0 and 1, this is due to the underlying precision in
Keras, which is sets the smallest non--zero float to be $10^{-7}$. The hit
...
with the underlying precision in Keras means that little can be done to optimise
the performance by varying the threshold in this case.
