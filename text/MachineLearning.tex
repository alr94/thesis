...
Instead, in this chapter we will briefly describe the theory behind NNs, as 
well as providing details of the techniques used in the subsequent chapters. 
...
nodes contains no cycles. The layers of an MLP are often \emph{fully 
connected} or \emph{dense}, meaning that the output of every node is connected 
to the input of every node in the next layer of the network. An example of a 
fully connected MLP, with two hidden layers, is shown in Figure \ref{fig:mlp}. 
...
performance of a network, as the new layers could be set to the identity, 
which would result in the same output from the network. However, it has been 
demonstrated that adding too many layers to a simple neural network will 
reduce performance, suggesting that the optimisation of larger networks leads 
to degradation\cite{He_2016_CVPR}.
...
for a given input. The backpropagation algorithm provides an means for 
...
considering the impact of the choice of activation function on the rate of
learning, these are the computational cost of calculating the derivative,
