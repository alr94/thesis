...
A LArTPC consists of a large volume of highly--purified liquid argon that is
...
\begin{figure}
	...
	wires that record the signal. The scintillation photons are collected by 
...
\subsection{\protodune{} Goals}
\protodune{} is one of two large scale prototypes for the DUNE Far Detector
modules that focusses on the single--phase LArTPC technology. The DUNE Far
...
\begin{itemize}[noitemsep, topsep=2pt]
	...
	\item A photon detection system (PDS) that is integrated into the APAs
...
plane at the center of the active width is flanked by two anode planes that 
...
two very similar designs of this style that differ only in terms of the 
...
by the emission of green light that is transmitted down the waveguide.

The ARAPUCA is a novel design that is only used in a small number of photon 
...
that is highly reflective on the inside. On one side of the box is the 
...
cosmic--ray muons that travel parallel to the anode plane, which are
particularly useful for calibration studies. In addition, the CRT can provide 
accurate timing information for any tracks that are successfully matched 
...
\begin{figure}
	...
	software components that are part of the ArtDAQ framework highlighted by 
...
orchestrator. There are multiple board readers that are each responsible for 
...
Particles that propagate through the detector, leave energy deposits in the 
...
First, the TPC waveforms are passed through a set of filtering algorithms that
...
track that crosses either a CPA or APA will produce track segments on either 
...
being labelled as clear cosmic--rays. These hits are formed into 3D slices that
...
\begin{itemize}
	\item Analysis processes that decode and analyse the raw data from each 
		detector subsystem
	\item Merging processes that collate monitoring data from each subsystem
	\item A web interface that displays monitoring data
\end{itemize}
...
The data that is passed to the OM processes is then split up and decoded by 
...
into the OM to a manageable level. This is followed by a second filter that 
...
Fragments that have to be decoded before they can be used by the OM. 
...
consideration. Each readout system defines a class that details the 
...
number of Analyser Modules that analyse the data from different detector
...
displays that showed the data from all APAs either side of the TPC as a 
...
\begin{figure}
	...
	TPC. An example of a data defect that was discovered in the OM can be seen 
...
\begin{figure}
	...
	\begin{subfigure}[b]{0.75\textwidth}
		...
		\caption {Waveforms for a single PDS channel that have been accumulated
...
rate during triggered operation. Trigger records are 2D histograms that 
...
\begin{figure}
	...
	\begin{subfigure}[b]{0.8\textwidth}
		...
		\caption {Trigger records. A 2D histogram that shows the active trigger
...
were continually overwritten on a time schedule that was decoupled from the 
...
Monet consists of a Flask web application\cite{flask} that uses 
...
display server that decides what to do with it. Depending on the nature of 
...
can contain any number of plots that are organised into an array on the screen,
