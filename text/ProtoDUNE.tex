...
and 2D deconvolution technique applied to a 7 GeV test beam event in 
...
other based on the arrival time of the corresponding particle. The $t_0$ for 
that track is equal to half the time shift required to realign the two segments 
into a continuous track. After the cosmic pass any clear cosmic--ray candidates 
are removed under the following conditions \cite{protoduneperf}:
\begin{itemize}
...
the wire spacing divided by the sine of the angle between the track and the
wire when the track is projected onto the wire plane. 
...
\begin{itemize}
	...
	\item Corrections in the direction of the wire planes, YZ corrections.
		Examples include wire to wire response, transverse diffusion, and detector
		features such as electron diverters near the APA boundaries.
\end{itemize}
...
recombination at the source to convert $dQ/dx$ to $dE/dx$. The conversion is 
based on the modelling of the recombination effect in liquid argon using the 
modified box model, which has been studied in a LArTPC by the ArgoNeuT
experiment \cite{Acciarri2013a}. The reconstructed $dE/dx$ is 
\begin{equation}
...
\(\beta = 0.212 \mbox{ (kV/cm)(g/cm}^3)} / \mbox{MeV}\) are the box model 
