\chapter{\label{ch:chargeid}Charge Classification with Convolutional Neural 
Networks} 
...
The stronger classification by the CNN gives supplementary information to
...
In practice, the CNN will be used to select events based on choosing events
...
on the CNN score of each individual hit. The ratio of data to simulation for 
all hits is on the order of 4\%. This ratio varies depending on the particle 
species, electrons have the smallest difference at around 1\%, while protons 
and cosmic--rays have an difference of 18\% based on this simple hit--by--hit 
algorithm.  
\begin{table}
	\centering
	\bgroup 
