...
(Pontecorvo, Maki, Nakagawa, and Sakata) matrix. For 3 flavour mixing the PMNS
...
% TODO: Put this discussion later, just do it in terms of U's for now
The PMNS matrix is often parametrised in terms of three mixing angles 
\(\theta_{12}\), \(\theta_{13}\), and \(\theta_{23}\) and three CP--violating 
phases \(\delta_{CP}\), \(\alpha_1\), and \(\alpha_2\):
\begin{equation}
...
components, which are responsible for oscillations of different initial states. 
...
as the TODO matrix. This component depends on \(\theta_{13}\) and 
\(\delta_{CP}\).

The final matrix in factorised version of the PMNS matrix is called the Majorana
component, it contains the Majorana CP--violating phases. 
